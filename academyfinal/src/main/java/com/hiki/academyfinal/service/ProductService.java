...
public class ProductService {
    ...
    public void registerProduct(ProductAddRequestVO VO, List<MultipartFile> images, List<String> types) throws IOException {
        ...
        for (VolumeInputVO input : VO.getVolumes()) {
            ...
            VolumeDto volume = VolumeDto.builder()
                ...
                .volumePrice(originPrice)
                .discountedVolumePrice(discounted)
                .build();
    ...
    public void updateProduct(long productNo, ProductAddRequestVO VO, MultipartFile image) throws IOException {
        ...
        List<VolumeDto> existingVolumes = volumeDao.selectByProductNo(productNo);
        List<String> updatedMlList = VO.getVolumes().stream()
            .map(v -> v.getVolumeMl().replaceAll("[^0-9]", "")) // 숫자만 추출
            .collect(Collectors.toList());

        for (VolumeDto existing : existingVolumes) {
            String mlNumeric = existing.getVolumeMl().replaceAll("[^0-9]", "");
            if (!updatedMlList.contains(mlNumeric)) {
                cartDao.deleteByVolumeNo(existing.getVolumeNo());
            }
        }
